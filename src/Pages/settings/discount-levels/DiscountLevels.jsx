...
const getDataApi = "/discount-levels";
const deleteDataApi = "/discount-level";
const sortUpApi = "/discount-level-sort-up";
const sortDownApi = "/discount-level-sort-down";

export default function DiscountLevels({ RenderFilterTabs }) {
  ...
  const tableHeaders = [
    {
      ...
      label: "Discount Name",
      render: (row) => (
        <span className="text-nowrap">{row.discountName || "-"}</span>
      ),
    },
      ...
      render: (row) => (
        <div className="me-4 flex justify-end items-center gap-2">
          ...
          <Button
            ...
            size="sm"
            onClick={() => {
              updateParam("discount-level-id", row.id);
  ...
  const handleSortBy = (sortColumn, sortDirection) => {
    getDiscountLevels(sortColumn, sortDirection);
  };

  const getDiscountLevels = useCallback(
    async (sortBy = null, sortDirection = null) => {
      try {
        ...
        if (data?.success) {
          ...
          notify(
            data.message || "Failed to fetch discount levels",
            "error",
            3000,
          );
        }
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (data?.success) {
        notify("Discount level deleted successfully", "success", 3000);
        getDiscountLevels();
      } else {
        notify(
          data.message || "Failed to delete discount level",
          "error",
          3000,
        );
      }
